...
<div class="post">
    ...
    <div class="entry">
<p>You can see all the GSN servers currently in the system. The last column gives you information whether this GSN is currently active or not.</p>
...
<?php
	...
	$rawData=Yii::app()->db->createCommand('SELECT g.*, case when p.gsn_id is not null then 1 else 0 end as priv_exists, case when p.gsn_id is not null then p.is_active else \'0\' end as is_active_priv FROM di_gsn g LEFT JOIN di_gsn_privileges p on p.gsn_id = g.gsn_id and p.user_id = '.Yii::app()->user->id." WHERE g.is_dummy = '0'")->queryAll();
	...
	'columns'=>array(		
		...
		array('header'=>'GSN activity','value'=>'$data["is_active"] == 1 ? "Active" : "Inactive"'),
                array('header'=>'Privilege given','value'=>'($data["is_active_priv"] == 1 ? "Yes" : "No")'),
		array(
				'class'=>'CLinkColumn',
