...
/**
 * This is the model class for table "prod_email_notifications".
 *
 * The followings are the available columns in table 'prod_email_notifications':
 ...
 */
class ProdEmailNotifications extends CActiveRecord {
...
//                $this->is_active = '0';
	...
	$sensor_information = "Sensor @ " . $gsn_data['gsn_name'] . " - " . $gsn_data['city'] . " " .
	 ...
	 $xml = '<?xml version="1.0" encoding="UTF-8"?>' . "\n" .
                ...
                '                    //data definition' . "\n" .
                '' . "\n" .
                '                    def delay = ' . $email_notification['resending_interval'] . '; //time in miliseconds between resending emails' . "\n" .
                ...
                '                    def criticalValue = ' . $email_notification['critical_value'] . ';' . "\n" .
                '                    def measuringUnit = "' . $email_notification['unit_name'] . '";' . "\n" .
                '' . "\n" .
                '' . "\n" .
                ...
                '                                  break;' . "\n" .
                '' . "\n" .
                '                          case 1: if (' . $email_notification['unit_name_upper'] . '<criticalValue){' . "\n" .
                ...
                '    <output-structure />' . "\n" .
                '  </processing-class>' . "\n" .
                '  <description>Automatic notification VS (type 1) for sensor ' . $sensor_data['sensor_name'] . ' and user ' . $user_data['username'] . '. Triggered on less then critical value.</description>' . "\n" .
                '  <addressing />' . "\n" .
                ...
                '          <predicate key="query">select * from ' . $sensor_data['sensor_name'] . '</predicate>' . "\n" .
                '        </address>' . "\n" .
                '        <query>select * from wrapper</query>' . "\n" .
                '      </source>' . "\n" .
                '      <query>select ' . $email_notification['unit_name'] . ', timed from source1 where ' . $email_notification['unit_name'] . '&lt;' . $email_notification['critical_value'] . '</query>' . "\n" .
                '    </stream>' . "\n" .
                '  </streams>*/' . "\n" .
                '</virtual-sensor>';
	return $xml;
    ...
    /**
     * Email notification generation for emails above critical level
     * @param <type> $id
     * @return string
     */
	...
	$sensor_information = "Sensor @ " . $gsn_data['gsn_name'] . " - " . $gsn_data['city'] . " " .
		...
		", longitude " . $sensor_data['location_x'];
          ...
          '    <output-structure />' . "\n" .
          '  </processing-class>' . "\n" .
          '  <description>Automatic notification VS (type 2) for sensor ' . $sensor_data['sensor_name'] . ' and user ' . $user_data['username'] . '. Triggered on greater then critical value.</description>' . "\n" .
          '  <addressing />' . "\n" .
          '  <storage history-size="1" />' . "\n" .
          '  <streams>' . "\n" .
          '    <stream name="stream1">' . "\n" .
